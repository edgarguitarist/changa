...
function countTries() {
  counter--;
  document.getElementById("tries").innerHTML = "Intentos Restantes: " + counter;
  if (counter <= 0) {
...
function keyup(elm, focus) {
  ...
  if (row - 2 >= 0 && row - 2 < maxrows && col - 1 >= 0 && col - 1 < maxcols) {
    T = table.rows[row - 2].cells[col - 1].children.length > 0;
  } //(T) cell above?
  if (row - 1 >= 0 && row - 1 < maxrows && col >= 0 && col < maxcols) {
    R = table.rows[row - 1].cells[col].children.length > 0;
  } //(R) cell to right?
  if (row >= 0 && row < maxrows && col - 1 >= 0 && col - 1 < maxcols) {
    B = table.rows[row].cells[col - 1].children.length > 0;
  } //(B) cell below?
  if (row - 1 >= 0 && row - 1 < maxrows && col - 2 >= 0 && col - 2 < maxcols) {
    L = table.rows[row - 1].cells[col - 2].children.length > 0;
  } //(L) cell to left?

  if (T == false && R == true && B == true && L == true && dir == "down") {
    document.getElementById("direction").innerHTML = "Verticalmente";
    dir = "down";
  } //top down
  if (T == true && R == false && B == true && L == false && dir == "down") {
    document.getElementById("direction").innerHTML = "Verticalmente";
    dir = "down";
  } //down
  if (T == true && B == false && dir == "down") {
    return;
  } //bottom down

  if (T == true && R == true && B == true && L == false && dir == "across") {
    document.getElementById("direction").innerHTML = "Horizontalmente";
    dir = "across";
  } //left across
  if (L == true && R == false && dir == "across") {
    return;
  } //end across

  if (B == true && L == false && focus == true) {
    document.getElementById("direction").innerHTML = "Verticalmente";
    dir = "down";
  }
  if (R == true && T == false && focus == true) {
    document.getElementById("direction").innerHTML = "Horizontalmente";
    dir = "across";
  }
...
//     focus;
  ...
  if (dir == "down") {
    table.rows[row].cells[col - 1].children[0].focus();
  }
  if (dir == "across") {
    table.rows[row - 1].cells[col].children[0].focus();
  }
}
...
window.addEventListener(
  ...
  function () {
    document
      ...
      .addEventListener("dblclick", direction);

    function direction() {
      if ((L == false && R == false) || (T == false && B == false)) {
        return;
      } //not an intersection
      if (dir == "across") {
        dir = "down";
      ...
      let dires = dir == "across" ? "Horizontalmente" : "Verticalmente";
      document.getElementById("direction").innerHTML = dires;
    }
  },
  false
);
...
window.onload = function () {
  ...
  //var words = ["flintstones", "simpson", "familyguy"];
  /*var clues = [
     "Stone age family name",
     "Homer's last name",
     "Peter Griffin's show (2 words)"
     ];*/
...
function saveScore() {
  alert("Felicidades gracias por completar el Crucigrama");
  if (!teacher) {
    $.ajax({
      url: "../scores.php",
      type: "POST",
...
function checkAnswer() {
  ...
  for (i = 0; i < answers.length; i++) {
    if (
      ...
      answers[i].getAttribute("data-answer").toLowerCase()
    ) {
  ...
  if (totalErrors == 0) {
    document.getElementById("tries").innerHTML = "Felicidades, has ganado";
    document.getElementById("showBtn").disabled = false;
    document.getElementById("showBtn").style.background = "#0065ba";
    document.getElementById("showBtn").style.color = "white";
    document.getElementById("checkBtn").style.background = "grey";
...
function Crossword(words_in, clues_in) {
  ...
  // constructor
  if (words_in.length < 2) throw "Un crucigrama debe tener al menos 2 palabras";
  if (words_in.length != clues_in.length)
    throw "El número de palabras debe ser igual al número de pistas.";
    ...
    for (var r = 0; r < grid.length; r++) {
      ...
      for (var c = 0; c < grid[r].length; c++) {
        ...
        if (cell == null) {
          ...
          var char = cell["char"];
          answer = char;
          //html.push("<input type='text' maxlength='1' />");
          var css_class = "";
        ...
        if (is_start_of_word) {
          ...
          html.push(
            "<td class='" +
              css_class +
              "' style=\"background-size: 15px; background-image:url('" +
              img_url +
              "')\">"
          );
          label++;
        ...
        if (cell != null) {
          if (show_answers) {
            ...
            html.push(
              "<input type='text' data-row='" +
                rownum +
                "' data-col='" +
                colnum +
                "' maxlength='1' onfocus='cellFocus(this)' onkeyup='keyup(this,false)' data-answer='" +
                answer +
                "' />"
            );
          }
